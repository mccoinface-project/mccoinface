...
The Mccoinface Core daemon can be configured to act as a trusted "border
router", implementing the mccoinface wire protocol and relay, making
...
The ZeroMQ feature in Mccoinface Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building mccoinfaced:
...
For instance:
    ...
    $ mccoinfaced -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/mccoinfaced.tx.raw \
...
These options can also be provided in mccoinface.conf.
...
From the perspective of mccoinfaced, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into mccoinfaced directly. Furthermore, no information is
...
using. Mccoinfaced appends an up-counting sequence number to each
