    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional MCF outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes MCF and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::MCF_TO_MCF) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a MCF-to-MCF transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::MCF_ONLY;
        ...
        // If MCF-to-MCF fails, create a peg-out transaction
