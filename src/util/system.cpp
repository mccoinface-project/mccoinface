...
const char * const BITCOIN_CONF_FILENAME = "mccoinface.conf";
...
#else
    const char* pszModule = "mccoinface";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mccoinface";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Mccoinface";
#else
    // Unix-like
    return pathRet / ".mccoinface";
#endif
#endif
}
